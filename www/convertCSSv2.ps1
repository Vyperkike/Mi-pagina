...
Write-Host "Ejecutando conversion mejorada de CSS a responsivos..." -ForegroundColor Green
...
foreach ($file in $cssFiles) {
    ...
    if (Test-Path $filePath) {
        ...
        if ($newContent -notmatch '@media\(min-width: 601px\)') {
            $newContent += "`n`n/* TABLET - 601px a 1024px */`n@media(min-width: 601px) and (max-width: 1024px) {}  `n/* MOBILE - menos de 600px */`n@media(max-width: 600px) {}`n"
        }
...
Write-Host "`nCompleto: Se actualizaron $actualizados archivos CSS" -ForegroundColor Cyan
